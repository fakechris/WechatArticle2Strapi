...
#### ✨ 核心功能
- **独立编辑器**: 全新的网页编辑器界面，支持可视化编辑和Markdown源码模式
- **富文本编辑**: 集成Quill.js富文本编辑器，支持标题、列表、链接、图片等
- **实时预览**: 可视化和源码模式之间无缝切换，实时预览效果
- **智能导入**: 支持HTML和Markdown内容导入，自动格式转换

#### 🛡️ 安全架构
- **代理服务器**: 浏览器与Strapi之间通过本地代理服务器通信，确保API token安全
- **配置隔离**: .articlerc.json配置文件在服务器端加载，浏览器端不暴露敏感信息
- **CORS处理**: 内置CORS支持，解决跨域访问问题

#### 🏷️ 智能Slug生成
- **服务器端生成**: 使用与CLI相同的transliteration库进行中文转拼音
- **长度限制**: 智能控制slug长度（默认50字符），支持单词边界断词
- **实时生成**: 标题输入时自动生成对应slug
- **降级机制**: 服务器不可用时自动使用客户端fallback方案

#### 🔄 字段映射集成
- **配置同步**: 直接使用.articlerc.json中的fieldMapping配置
- **字段转换**: 自动将description映射为summary等，与CLI保持一致
- **预设字段**: 支持fieldPresets中的预设值（如news_source: "reprint"）
- **动态加载**: 从服务器动态获取字段映射配置

#### 📝 草稿模式实现
- **双重策略**: 使用?status=draft查询参数 + 移除publishedAt字段
- **降级处理**: draft端点失败时自动使用常规端点
- **一致性**: 与CLI草稿模式逻辑完全一致

#### 🖼️ 图片处理增强
- **多种上传方式**: 支持Strapi媒体库上传和Base64嵌入
- **智能处理**: 粘贴图片时自动处理，支持从剪贴板直接插入
- **格式转换**: 自动处理不同图片格式和来源

#### 🔧 技术架构
- **Node.js服务器**: 使用Node.js构建的本地代理服务器
- **配置加载**: 支持从命令行参数加载.articlerc.json配置文件
- **API端点**: 提供配置、字段映射、slug生成等多个API端点
- **错误处理**: 完善的错误处理和日志记录机制
...
| 版本 | 内容纯度 | 处理速度 | 图片精度 | 界面干净度 | 元数据完整性 | 编辑器模式 |
|------|----------|----------|----------|------------|--------------|------------|
...
| v0.5.2 | 95% | 16ms | 85% | +100% | +220% | ❌ |
| v0.6.0 | 95% | 16ms | 85% | +100% | +220% | ✅ |
